...
\begin{exmp}{Sketch the graph of $f(x) = - 2x^2 + x + 6$}{}
  Because $a$ is negative, the parabola is upside-down. The vertex is at
  \begin{gather}
...
\begin{figure}[t]
  \centering
  \subbottom[\(x^3 + x^2 + x\): No vertex\label{fig:cubic_no_vertex}]{
    \begin{minipage}{0.4\textwidth}
      \centering
      \includegraphics{geometryandcalculus/cubic_novertex.pdf}
    \end{minipage}
  }
  \subbottom[\(x^3\): One vertex\label{fig:cubic_one_vertex}]{
    \begin{minipage}{0.4\textwidth}
      \centering
...
\begin{figure}[ht]
  \centering
  \subbottom[\vphantom{\(\frac{1}{4}\)}\(x^3 + x\): 3 roots\label{fig:cubic_two_vertices_threeroots}]{
    \includegraphics{geometryandcalculus/cubic_twovertex_threeroots.pdf}
  }
  \subbottom[\(x^3 - \frac{3}{4}x + \frac{1}{4}\): 2 roots\label{fig:cubic_two_vertices_tworoots}]{
    \includegraphics{geometryandcalculus/cubic_twovertex_tworoots.pdf}
  }
  \subbottom[\vphantom{\(\frac{1}{4}\)}\(x^3 - x\): 1 root\label{fig:cubic_two_vertices_oneroot}]{
...
\begin{figure}[ht]
  \centering
  \subbottom[Unfolded. Gray part is cut out.\label{fig:optimization_box_volume_unfold}]{
    \includegraphics[]{geometryandcalculus/optimization_box_volume_unfold.pdf}
  }
  \subbottom[Folded\label{fig:optimization_box_volume_folded}]{
    \includegraphics[]{geometryandcalculus/optimization_box_volume_folded.pdf}
  }
  \caption{Box volume optimization}
\end{figure}

Shown in \cref{fig:optimization_box_volume_unfold}, if we cut out a square side length $x$ from all corners, then the side length of the final box will be $a - 2x$, $b - 2x$, and $x$. The volume $V(x)$, is then
\begin{equation}
...
Polynomials appears everywhere. Most of the time, you're required to find its root. But it might come as a surprise that it is \emph{impossible} to find a general formula for finding the root of polynomials with degrees higher than four.\footnote{This is a consequence of the Galois theory, which is a profound result from abstract algebra. You'd usually learn in graduate mathematics, but if you want to just scratch the surface, the video by \emph{Math Visualized}, on \emph{Galois Theory Explained Simply} \cite{galois-explained-2020}, does explain it in brief. Still, I had to watch it a couple of times before understanding it.} So most of the time, we resort to root approximation methods.
...
For a function that you can't find a derivative, or you're just too lazy to find the derivative explicitly, you can turn the Newton-Raphson method into a numerical algorithm via the definition of derivative. From \cref{eq:newton-raphson-method},
\begin{align}
  x_{n + 1} & = x_n - \frac{f(x_n)}{\lim_{h \appr 0}\frac{f(x_n + h) - f(x_n)}{h}} \\
            & = x_n - \lim_{h \appr 0}\frac{f(x_n)h}{f(x_n + h) - f(x_n)},
\end{align}
where $h$ can be set to any arbitrarily small values, e.g., $0.01$. If you have some experiences with coding, this can be easily implemented by using a \texttt{for} loop. Here's a simple python snippet that I've written to do just that. Just put in the function in \texttt{def f(x):} block, put the desired iteration in \texttt{iterationCount}, your first guess in \texttt{firstGuess}, and the value of $h$ in \texttt{derivativeStep}. If you have some experiences, I highly encourage you to tinker around with this algorithm, and you might find something beautiful.
...
Alongside the tangent line, there is another line that we're interested in: the normal. A normal line is a line that's perpendicular to the tangent. By analytical geometry, two lines are perpendicular \ifft the product of their slope is $-1$. Let the slope of the tangent be $m_T$ and the normal be $m_N$, then
\begin{align}
  m_T \times m_N & = -1              \\
  m_N            & = -\frac{1}{m_T}.
\end{align}
...
\begin{align}
  \int_{x = a}^{x = b}\sqrt{\odif{x}^2 + \odif{y}^2}
   & = \int_{x = a}^{x = b}\sqrt{1 + \ab(\odv{y}{x})^2}\odif{x}                           \\
   & = \int_{x = a}^{x = b}\sqrt{1 + \ab(\odv{f(x)}{x})^2}\odif{x}. \label{eq:arc-length}
\end{align}
