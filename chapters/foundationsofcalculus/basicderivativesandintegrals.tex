...
\subsubsection{Explicit differentiation}
...
\begin{exmp}{$\odv*{\ab(\ln\ab(\sqrt{\frac{x + 1}{x - 1}}))}{x}$}{diff-technique-4}
	Breaking this derivative with the chain rule will be a nightmare. As said, let's use the algebraical properties of logarithm to help us with this. Namely, $\ln(\frac{a}{b}) = \ln(a) - \ln(b)$, and $\ln(a^b) = b\ln(a)$.
	\begin{align}
		\odv*{\ab(\ln(\sqrt{\frac{x + 1}{x - 1}}))}{x} & = \odv*{\ab(\ln(\sqrt{x + 1}) - \ln(\sqrt{x - 1}))}{x}                                 \\
