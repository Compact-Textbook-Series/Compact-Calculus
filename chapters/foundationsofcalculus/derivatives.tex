...
As said in the preface, I have thrown most unrelated contents into the appendices. But as much as I'd love to do that, some purely mathematical contents and exercises are still essential for understanding and solving dynamical systems. While I've kept those in, I tried to make them as brief as possible for it to not disrupt your entertainment.
...
If $a$ is a variable, then $\odif{a}$ is a very small quantity of $a$, known as an \textbf{infinitesimal}. E.g., if $\vv{x}$ is displacement, then $\odif{\vv{x}}$ is a very small displacement. If $t$ is time, then $\odif{t}$, a very short time. It's very similar to how we usually write a difference in a variable $x$, $\Dd{x}$. Replacing the $\symrm{\Delta}$ with $\symrm{d}$ conveys that \enquote{it is a very small difference in $x$.}

Infinitesimals can also be divided to yield a ratio between infinitesimals. E.g., the ratio between some small distance and some short time, $\odv{\vv{x}}{t}$ is the speed $\vv{v}$.

\section{Speed and instantaneous rate of change}
\label{sec:speedandinstantaneousrateofchange}

Our first dynamical system that I'd like to discuss is the car; specifically the \textbf{speedometer}, which displays the speed of the car at an instance in time. How does it do that?

If we're traveling at a speed $\qty{5}{\meter\per\second}$, when \emph{exactly} are we traveling at $\qty{5}{\meter\per\second}$? You could say $\vv{v} = \qty{5}{\meter}$ \emph{at the moment of measurement}. But what exactly is \emph{at the moment of measurement?} To measure something is to observe, which is to use a measurement tool to \emph{capture} its current state. So, measurement in this case, is analogous to capturing the picture of a car at that moment. But if we say, \emph{at the moment of measurment,} it's like saying \enquote{Oh, I can find the speed of the car by just taking a picture of it.} But that's illegal! To calculate speed, we have to compare two points in space through time. Or, the rate of change of distance through time:
\begin{equation}
...
\begin{equation*}
	\vv{a}(t) = \odv{\vv{v}(t)}{t}.
\end{equation*}
\begin{wraptable}[12]{l}{0.36\textwidth}
	\begin{tabular}{c | c}
		Order & Name           \\
		\hline
		1     & Velocity/Speed \\
