...
The derivative of sine is cosine. We'll prove it later, but the geometrical interpretation is that the derivative of sine oscillates along off-sync with the function itself. When \(x = 0\), there's a \(\qty{45}{\degree}\) rising slope. At \(x = \frac{\cpi}{2}\), the slope flattens down to zero. Later at \(x = \cpi\), the slope is going \(\qty{45}{\degree}\) down, etc. If we plot the slope of the sine function at each point (shown as dots in \cref{fig:sine_cosine_relation}), it resembles a cosine curve.
...
To prove this, let's start with the definition of the derivative given in \cref{eq:derivatives-derivative_definition},
\begin{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    \odv*[order = 2]{\ab(\sin(x))}{x} & = \odv*{\ab(\cos(x))}{x} = -\sin(x)  \\
    \odv*[order = 3]{\ab(\sin(x))}{x} & = \odv*{\ab(-\sin(x))}{x} = -\cos(x) \\
    \odv*[order = 4]{\ab(\sin(x))}{x} & = \odv*{\ab(-\cos(x))}{x} = \sin(x)
  \end{aligned}
\end{equation}
...
Altogether, I've summarized all the derivatives of trigonometric functions into \cref{tab:derivative-trigonometric-functions}.
\begin{table}[ht]
  \begin{center}
    \begin{tblr}{C | L}
      f(x)    & \odv*{f(x)}{x}  \\
  ...
  \label{tab:derivative-trigonometric-functions}
\end{table}
...
\begin{equation}
  \ddot{x} = \odv{\dot{x}}{t} = \odv*{\ab(-\omega A\sin(\omega t))}{t} = -\omega^2A\sin(\omega t). \label{eq:harmonic-oscillator-acceleration}
\end{equation}
...
\begin{align}
  ...
  0 & = \begin{multlined}[t]
          \ab(2a_2 + 3\cdot 2a_3t + 4\cdot 3a_4t^2 + 5\cdot 4a_5t^3 + 6\cdot 5a_6t^4 + 6\cdot 7a_7t^5 + \dots) \\
          + \ab(\omega^2A + a_2\omega^2t^2 + a_3\omega^2t^3 + a_4\omega^2t^4 + a_5\omega^2t^5\dots) \nonumber
        \end{multlined} \\
  0 & = \begin{aligned}[t]
           & \ab(2a_2 + \omega^2A)                   \\
           & +(3 \cdot 2a_3)t                        \\
...
Since the velocity function (\cref{eq:harmonic-oscillator-velocity}) is a sine function, it must oscillate \(\cpi\) radians off-phase relative to the position function. Therefore,
\begin{enumerate}
...
If you don't know much about units, it's fine if you skip these paragraphs straightaway. But one clue that suggests the physical meaning of \(\omega\) is its unit. Since \(\omega = \sqrt{\frac{k}{m}}\), we have to analyze the units of its components separately. \(k\) is a spring stiffness constant which is got from
\begin{equation}
...
\end{equation}
Funnily enough, this is the unit of Hertz, (\(\frac{1}{\unit{\second}} = \unit{\hertz}\)) which describes frequencies. But it cannot possibly be just a unit of frequency because the solution of the differential equation says so.
...
Another way \(\omega\) can be interpreted is by looking at the expression of \(\omega\) itself: \(\sqrt{\frac{k}{m}}\). The spring stiffness constant \(k\) is on the top, so it means the stiffer the spring, the higher omega is. A stiffer spring would allow the spring to pull the mass faster; thus, increasing the frequencies. The mass \(m\) is on the bottom: the larger the mass, the lower the omega. That just means a more massive block is harder for the spring to move; therefore, the mass will travel slower, decreasing the frequencies.
...
\begin{align}
  \odv[ord = 2]{x(t)}{t}
                         & = \odv*[ord = 2]{\ab(A\cos(\omega t) + B\sin(\omega t))}{t}                                        \\
...
\begin{align}
  x(0) & = A\cos\ab(\omega(0)) + B\sin\ab(\omega(0))       \\
  X    & = A\cancelto{1}{\cos(0)} + B\cancelto{0}{\sin(0)} \\
  X    & = A
\end{align}
...
\begin{align}
  v(t = 0)         & = -\omega X\sin\ab(\omega (0)) + \omega B\cos\ab(\omega (0)) \\
  \frac{V}{\omega} & = - \cancelto{0}{X\sin(0)} + B\cancelto{1}{\cos(0)}          \\
  \frac{V}{\omega} & = B
\end{align}
Therefore,
\begin{equation}
  x(t) = X\cos(\omega t) + \frac{V}{\omega}\sin(\omega t). \label{eq:harmonic-oscillator-other-solutions-0}
\end{equation}

In this form, it's quite hard to see what is the amplitude and the point of maximum and minimum velocity is. To make it easier for us to interpret, we can combine the cosine and sine term using the \textbf{harmonic addition theorem}
\begin{equation}
  a\cos\theta + b\sin\theta = c\cos(\theta + \phi)
\end{equation}
where
\begin{gather}
  c = \sgn(a)\sqrt{a^2 + b^2} \mathand \phi = \arctan\ab(-\frac{b}{a}).
\end{gather}
\Cref{eq:harmonic-oscillator-other-solutions-0} then becomes
\begin{equation}
  \sqrt{X^2 + \frac{V^2}{\omega^2}}\cos\ab(\omega t + \arctan\ab(-\frac{V}{\omega X}))
\end{equation}
In this form, it's easy to see that the amplitude: the point that the mass is furthest away from the spring, is \(x = \sqrt{X^2 + \frac{V^2}{\omega^2}}\). But what is that \(\arctan\ab(-\frac{V}{\omega X})\) doing there? What does it represent? We shall discuss this in detail in \cref{sec:phase-space}

\section{Trigonometric substitutions}
\label{sec:trigonometric_substitution}
...
\begin{displayquote}
  \enquote{\emph{How in the world is it possible? A simple substitution could do such wonderful things! It just destroyed this integrals involving square roots and turned it into a much more managable form. Amazing!}}
\end{displayquote}
...
\begin{equation}
  \int f\ab(\sqrt{A^2 - x^2}, x)\odif{x}.
\end{equation}
where \(f\) is a function of \(\sqrt{A^2 - x^2}\) and \(x\). As said earlier, substitute \(x = a\sin\theta\); thus,
\begin{align}
  \odv{x}{\theta} & = \odv*{(A\sin\theta)}{\theta} \\
  \odif{x}        & = A\cos(\theta)\odif{\theta}.
\end{align}
Using \(1 - \cos^2\theta = \sin^2\theta\), our integral becomes
\begin{align}
   & A\int f\ab(\sqrt{A^2 - A^2\cos^2\theta}, A\sin\theta)\cos\theta\odif{\theta} \nonumber \\
   & = A\int f\ab(A\sqrt{1 - \cos^2\theta}, A\sin\theta)\cos\theta\odif{\theta}             \\
   & = A\int f\ab(A\sqrt{\sin^2\theta}, A\sin\theta)\cos\theta\odif{\theta}                 \\
   & = A\int f\ab(A\sin\theta, \sin\theta)\cos\theta\odif{\theta}
\end{align}
Now let's see some magic: two magic tricks with circles.

The next two examples evaluates the perimeter and area of a circle by using trigonometric substitution. The equation of a circle radius \(R\) centered at the origin is given by \(x^2 + y^2 = R^2\), which is in an implicit form. To integrate this, we have to turn it into an explicit form
\begin{equation}
  y = f(x) = \sqrt{R^2 - x^2}.
\end{equation}
...
\begin{exmp}{Perimeter of a circle}{}
  This is an excuse for me to revisit the arc length formula in \cref{sec:arc-length}. The perimeter of a circle is essentially double of the arc length of \(\sqrt{R^2 - x^2}\) from \(-R\) to \(R\) which from \cref{eq:arc-length}, is given by
  \begin{equation}
    \int_{-R}^{R}\sqrt{1 + \ab(\odv{\sqrt{R^2 - x^2}}{x})^2}\odif{x}.
  \end{equation}
  The derivative inside can be evaluated using the chain rule:
  \begin{align}
    \odv{\sqrt{R^2 - x^2}}{x} & = \odv{\sqrt{u}}{u} \times \odv{u}{x}                &  & u = R^2 - x^2       \\
                              & = \odv{u^{\frac{1}{2}}}{u} \times \odv{R^2 - x^2}{x}                          \\
                              & = \frac{1}{2\sqrt{u}}(-2x)                           &  & \textrm{Power rule} \\
                              & = -\frac{x}{\sqrt{R^2 - x^2}}                        &  & u = R^2 - x^2
  \end{align}
  Our integral then becomes
  \begin{align}
    \int_{-R}^{R}\sqrt{1 + \ab(-\frac{x}{\sqrt{R^2 - x^2}})^2}\odif{x}
     & = \int_{-R}^{R}\sqrt{1 + \frac{x^2}{R^2 - x^2}}\odif{x}         \\
     & = \int_{-R}^{R}\sqrt{\frac{R^2 - x^2 + x^2}{R^2 - x^2}}\odif{x} \\
     & = R\int_{-R}^{R}\frac{1}{\sqrt{R^2 - x^2}}\odif{x}.
  \end{align}
  This form can be easily evaluated by substituting \(x = R\sin\theta, \odif{x} = R\cos\theta\odif{x}\). The bounds are then changed by \(\theta = \arcsin\ab(\frac{x}{R})\) to
  \begin{equation}
    \begin{gathered}
      x = -R \appr \theta = \arcsin\ab(\frac{-R}{R}) = -\frac{\cpi}{2}, \\
      x = R \appr \theta = \arcsin\ab(\frac{R}{R}) = \frac{\cpi}{2}.
    \end{gathered}
  \end{equation}
  Our integral then becomes
  \begin{align}
     & R\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\frac{1}{\sqrt{R^2 - R^2\sin^2\theta}}R\cos\theta\odif{\theta} \nonumber                             \\
     & = R\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\frac{1}{R\sqrt{1 - \sin^2\theta}}R\cos\theta\odif{\theta}                                         \\
     & = R\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\frac{\cos\theta}{\cos\theta}\odif{\theta} = R\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\odif{\theta} \\
     & = R(\theta|_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}} = R\ab(\frac{\cpi}{2} - \ab(-\frac{\cpi}{2})) = R\cpi;
  \end{align}
  therefore, the perimeter of a whole circle radius \(R\) is \(2\cpi R\).
\end{exmp}
\begin{exmp}{Area of a circle}{}
  The area of a circle radius \(R\) is double the integral of \(\sqrt{R^2 - x^2}\):
  \begin{equation}
    \int_{-R}^{R}\sqrt{R^2 - x^2}\odif{x}.
  \end{equation}
  Perform a trigonometric substitution similar to the example above: \(x = R\sin\theta\), \(\odif{x} = R\cos\theta\odif{\theta}\), \(x = -R \appr \theta = -\frac{\cpi}{2}\), and \(x = R \appr \theta = \frac{\cpi}{2}\). Our integral then becomes
  \begin{align}
    \int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\sqrt{R^2 - R^2\sin^2\theta}R\cos\theta\odif{\theta}
     & = \int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}R^2\sqrt{1 - \sin^2\theta}\cos\theta\odif{\theta} \nonumber \\
     & = R^2\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\cos^2\theta\odif{\theta}.
  \end{align}
  ...
  \begin{align}
    R^2\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\frac{\cos(2\theta) + 1}{2}\odif{\theta}
     & = \frac{R^2}{2}\ab[\int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\cos(2\theta)\odif{\theta} + \int_{-\frac{\cpi}{2}}^{\frac{\cpi}{2}}\odif{\theta}].
  \end{align}
  The second integral simply evaluates to \(\frac{\cpi}{2} - \ab(-\frac{\cpi}{2})\), which is just \(\cpi\). The first integral requires another substitution.
\end{exmp}

\subsection*{Integrals containing \(\sqrt{x^2 - A^2}\)}

Let the integral be described by
\begin{equation}
  \int f\ab(\sqrt{x^2 - A^2}, x)\odif{x}.
\end{equation}
We want to take advantage of \(\sec^2\theta - 1 = \tan^2\theta\), so substitute \(x = A\sec\theta\). From \cref{tab:derivative-trigonometric-functions},
\begin{align}
  \odv{x}{\theta} & = \odv*{(A\sec\theta)}{\theta}        \\
  \odif{x}        & = A\sec\theta\tan\theta\odif{\theta}.
\end{align}
Our integral then becomes
\begin{align}
   & \int f\ab(\sqrt{A^2\sec^2\theta - A^2}, A\sec\theta)A\sec\theta\tan\theta\odif{\theta} \nonumber \\
   & = A\int f\ab(A\sqrt{1 - \sec^2\theta}, A\sec\theta)\sec\theta\tan\theta\odif{\theta}             \\
   & = A\int f\ab(A\sqrt{\tan^2\theta}, A\sec\theta)\sec\theta\tan\theta\odif{\theta}                 \\
   & = A\int f\ab(A\tan\theta, A\sec\theta)\sec\theta\tan\theta\odif{\theta}.
\end{align}
...
\begin{equation}
  \int f\ab(\sqrt{A^2 + x^2}, x)\odif{x}.
\end{equation}
We want to take advantage of \(1 + \tan^2\theta = \sec^2\theta\), so substitute \(x = A\tan\theta\). From \cref{tab:derivative-trigonometric-functions},
\begin{align}
  \odv{x}{\theta} & = \odv*{(A\tan\theta)}{\theta} \\
  \odif{x}        & = A\sec^2\theta.
\end{align}
Our integral then becomes
\begin{align}
   & \int f\ab(\sqrt{A^2 + A^2\tan^2\theta}, A\tan\theta)A\sec^2\theta\odif{\theta} \nonumber \\
   & = \int f\ab(A\sqrt{1 + \tan^2\theta}, A\tan\theta)\sec^2\theta\odif{\theta}              \\
   & = \int f\ab(A\sqrt{\sec^2\theta}, A\tan\theta)\sec^2\theta\odif{\theta}                  \\
   & = \int f\ab(A\sec\theta, A\tan\theta)\sec^2\theta\odif{\theta}.
\end{align}
